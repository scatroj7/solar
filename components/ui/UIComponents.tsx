...
export const TabsTrigger = ({ className, children, active, onClick, disabled }: { className?: string, children?: ReactNode, active?: boolean, onClick?: () => void, disabled?: boolean }) => (
  <button
    type="button"
...
export const Logo = ({ className, variant = 'dark' }: { className?: string, variant?: 'dark' | 'light' }) => {
  const navyColor = variant === 'light' ? '#ffffff' : '#051532';
  return (
    <div className={cn("flex items-center gap-3 select-none", className)}>
      ...
      <div className="flex flex-col items-center justify-center -space-y-0.5">
        <h1 className={cn("text-2xl font-bold tracking-tight leading-none", variant === 'light' ? 'text-white' : 'text-[#051532]')}>
          Solar<span className="text-[#e96504]">Smart</span>
        </h1>
        <p className={cn("text-[9px] font-semibold tracking-[0.2em] w-full text-center mt-0.5 uppercase opacity-90", variant === 'light' ? 'text-slate-300' : 'text-slate-500')}>
          Geleceğin Enerjisi
        </p>
      </div>
    </div>
